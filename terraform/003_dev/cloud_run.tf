locals {
  cloud_run_image_tag = var.cloudrun_image_tag_dev != "" ? var.cloudrun_image_tag_dev : var.cloudrun_image_tag_default
...
resource "google_cloud_run_v2_service" "web_django" {
  ...
  template {
    ...
    service_account = google_service_account.web_dev.email
    volumes {
      name = "cloudsql"
      cloud_sql_instance {
...
resource "null_resource" "web_django_management_execution" {
  provisioner "local-exec" {
    command = "gcloud beta run jobs execute ${google_cloud_run_v2_job.web_django_management.name} --project ${data.google_project.web_dev.project_id} --region ${google_cloud_run_v2_job.web_django_management.location} --wait"
  }
}
